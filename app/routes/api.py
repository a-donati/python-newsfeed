...
# db.commit() performs the INSERT against the database and that db.rollback() discards the pending commit if it fails.
    db.add(newComment)
    db.commit()
  except:
    print(sys.exc_info()[0])
...
def update(id):
  ...
  try:
    # retrieve post and update title property
     # SQLAlchemy requires you to query the database for the corresponding record, update the record like you'd update a normal object, and then recommit it.
